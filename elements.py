...
class Name(Expr):
    ...
    def eval(self,psstacks):
        ...
        if n[0] == '/':
            psstacks.opPush(n)
        elif n in psstacks.builtin_operators:
            psstacks.builtin_operators[n]()
        elif psstacks.lookup(n) is not None:
            val = psstacks.lookup(n)
            if isinstance(val, CodeArray):
                val.apply(psstacks)
            else:
                psstacks.opPush(val)
                
        else:
            raise Exception(f'Error: unknown symbol: {n}')
